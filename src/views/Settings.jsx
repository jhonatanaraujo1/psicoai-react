  ...
  return (
    <div>
      ...
      <div style={{ background: 'var(--g700)', borderRadius: 'var(--r2)', padding: '24px', marginBottom: '16px', position: 'relative', overflow: 'hidden' }}>
        ...
        <div style={{ fontFamily: "'Fraunces', serif", fontSize: '26px', color: '#fff', fontWeight: 300, marginBottom: '3px' }}>Psic Notes — {planPrice}/mês</div>
...
function TabAjuda({ onOpenOnboarding, onOpenTermos }) {
  ...
  const FAQS = [
    { q: 'Como funciona a análise IA?', a: 'O Psic Notes lê suas anotações e devolve hipóteses diagnósticas DSM-5/CID-11 com probabilidade ponderada, padrões comportamentais detectados e alertas graduados — tudo baseado no que você mesmo registrou, sem automatismo.' },
    { q: 'A IA substitui meu diagnóstico?', a: 'Não. O Psic Notes é um assistente de raciocínio clínico. Todo output é explicitamente marcado como "suporte clínico" e o diagnóstico final é sempre responsabilidade exclusiva do psicólogo.' },
  ...
  return (
    <div>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '12px', marginBottom: '24px' }}>
        ...
        <div style={{ padding: '16px 18px', border: `1px solid ${status?.calendarSync ? '#CEEAD6' : 'var(--gr2)'}`, borderRadius: 'var(--r)', background: status?.calendarSync ? '#F6FEF8' : 'var(--w)', display: 'flex', alignItems: 'flex-start', gap: '14px', opacity: status?.connected ? 1 : 0.5 }}>
          ...
          <div style={{ flex: 1 }}>
            ...
            <div style={{ fontSize: '12px', color: 'var(--gr5)', lineHeight: 1.5 }}>
              Seus eventos do Google Agenda aparecem na Agenda do Psic Notes. Supervisões, reuniões e compromissos pessoais ficam visíveis junto com as sessões.
            </div>
          </div>
      ...
      <div style={{ padding: '12px 14px', background: 'var(--ow)', border: '1px solid var(--gr2)', borderRadius: 'var(--r)', fontSize: '12px', color: 'var(--gr5)', lineHeight: 1.6, display: 'flex', gap: '10px' }}>
        ...
        O Psic Notes solicita apenas permissões de agenda (criar eventos e ler calendário). Nenhum dado clínico é enviado ao Google. Você pode revogar o acesso a qualquer momento.
      </div>
    </div>
  )
}
